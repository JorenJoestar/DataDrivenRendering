        ...
        switch ( c ) {
            case 'i':
            {
                if ( lexer_expect_keyword( token.text, 7, "image1D" ) ||
                    ...
                    CodeFragment::Resource resource = { hydra::graphics::ResourceType::ImageRW, name_token.text };
        ...
        switch ( c ) {
            ...
            case 't':
            {
                if ( lexer_expect_keyword( token.text, 9, "texture1D" ) ||
                    ...
                    binding.type = hydra::graphics::ResourceType::ImageRW;
                    binding.start = 0;
                    binding.count = 1;
...
void declaration_pass_dispatch( Parser* parser, Pass& pass ) {
    ...
    pass.compute_dispatch.x = atof(token.text.text);
    ...
    pass.compute_dispatch.y = atof( token.text.text );
    ...
    pass.compute_dispatch.z = atof( token.text.text );
}
...
void declaration_includes( Parser* parser ) {
    ...
    while ( !lexer_equals_token( parser->lexer, token, Token::Token_CloseBrace ) ) {
        ...
        if ( token.type == Token::Token_String ) {
            ...
            char* text = hydra::file_read_into_memory( path_buffer.data, nullptr, false, *hydra::memory_get_system_allocator() );
            if ( text ) {
                Lexer lexer;
...
static void generate_glsl_and_defaults( const Shader& shader, StringBuffer& out_buffer, StringBuffer& out_defaults, const DataBuffer& data_buffer ) {
    ...
    out_defaults.append_m( &resource_type, sizeof( hydra::graphics::ResourceType::Enum ) );
...
//
//
static void append_reflection_data( const char* reflection_filename, const char* namespace_name, FILE* reflection_c_file, TypeDefinitionAlias* name_to_type, StringBuffer& buffer ) {

    if ( reflection_c_file ) {

        // Parse json
        using json = nlohmann::json;
        char* reflection_json = hydra::file_read_into_memory( reflection_filename, nullptr, true, *hydra::memory_get_system_allocator() );
        json parsed_json = json::parse( reflection_json );

        fprintf( reflection_c_file, "\tnamespace %s {\n\n", namespace_name );

        auto types = parsed_json[ "types" ];
        ...
        for ( json::iterator it = types.begin(); it != types.end(); ++it ) {
            ...
            //if ( string_hash_get() )
            char* name_interned = buffer.append_use( it.key().c_str() );
            char* type_interned = buffer.append_use( name_str.c_str() );
            string_hash_put( name_to_type, name_interned, type_interned );

            fprintf( reflection_c_file, "\t\tstruct %s {\n", name_str.c_str() );
            ...
            for ( u32 i = 0; i < members.size(); ++i ) {
                ...
                if ( name_str.compare( "mat4" ) == 0 ) {
                    ...
                    char* type_name = string_hash_get( name_to_type, name_str.c_str() );
                    if ( type_name ) {
                        name_str = type_name;
                    }
                    else {
                        hydra::print_format( "ERror parsing type %s\n", name_str.c_str() );
                    }
                }
                ...
                if ( offset.is_number() ) {
                    ...
                    if ( member_memory_offset < current_member_offset ) {
                        while ( member_memory_offset < current_member_offset ) {
                            ...
                            fprintf( reflection_c_file, "\t\t\t pad%3d;\n", padding_added++ );
                        }
                    }
                ...
                fprintf( reflection_c_file, "\t\t\t%s", name_str.c_str() );
                // Write name
                name.get_to( name_str );
                fprintf( reflection_c_file, "\t\t\t\t%s;\n", name_str.c_str() );
            }
            fprintf( reflection_c_file, "\t\t};\n\n" );
        }
...
static bool append_finalized_shader_code( const char* path, const CodeGenerator* code_generator, const Pass::ShaderStage& shader_stage,
                                          StringBuffer& filename_buffer, StringBuffer& code_buffer, const StringBuffer& constants_buffer, FILE* reflection_file ) {
    ...
    for ( size_t i = 0; i < code_fragment->includes.size(); i++ ) {
        ...
        if ( (flag & k_local_hfx_code_fragment_flag) == k_local_hfx_code_fragment_flag ) {
            ...
            char* include_code = hydra::file_read_into_memory( filename_buffer.data, nullptr, true, *hydra::memory_get_system_allocator() );
            if ( include_code ) {
                code_buffer.append_f( "%s\n", include_code );
    ...
    hydra::print_format( ">>>>>>> Compiling shader\n" );
    ...
    if ( (compile_options & CompileOptions_SpirV) == CompileOptions_SpirV ) {
        ...
        final_shader = hydra::file_read_into_memory( final_shader_filename, &final_shader_size, false, *hydra::memory_get_system_allocator() );
    }
    else {
        // Convert to SpirV binary blob.
        char* glsl_compiler_path = filename_buffer.append_use_f( "%sglslangValidator.exe", code_generator->shader_binaries_path );
    ...
    if ( !compilation_succeeded ) {
        // Print the shader
        char* shader_code = hydra::file_read_into_memory( temp_filename, nullptr, false, *hydra::memory_get_system_allocator() );
        ...
        error_string = strstr( error_string, ":" );
        ++error_string;
        int error_line = atoi( error_string );

        const int k_output_error_lines = 10;
        int min_line = max( 0, error_line - k_output_error_lines );
        // Search line
        char* shader_error_line = shader_code;
        for ( size_t i = 0; i < min_line; i++ ) {
            shader_error_line = strstr( shader_error_line, "\n" );
            shader_error_line += 2;
        }

        char* shader_next_error_line = strstr( shader_error_line, "\n" );
        shader_next_error_line += 2;
        int32_t shader_line_size = shader_next_error_line - shader_error_line;
        ...
        for ( size_t i = 0; i < k_output_error_lines; i++ ) {
            ...
            hydra::print_format( "%s%s", ( i == k_output_error_lines - 1 ) ? "ERROR LINE:\n" : "", shader_line_text );

            shader_error_line = shader_next_error_line;
            shader_next_error_line = strstr( shader_error_line, "\n" );
            shader_next_error_line += 2;
            
            shader_line_size = shader_next_error_line - shader_error_line;
        }

        hydra::print_format( "\n>>>>>>> Compilation ERROR!\n\n" );
        //hydra::print_format( "FULL CODE %s\n", shader_code);

        // NOTE: return!
        return false;
    }
    else {
        hydra::print_format( ">>>>>>> Compilation successful!\n\n" );
    }
    ...
    if ( generate_reflection_data ) {
        ...
        char* namespace_name = filename_buffer.append_use_f( "%s_%s", intermediate_shadername, s_shader_compiler_stage[ stage ] );
        append_reflection_data( reflection_filename, namespace_name, reflection_file, code_generator->name_to_type, filename_buffer );
    }
    ...
    if ( !keep_intermediate ) {
        ...
        remove( final_shader_filename );

        if ( reflection_filename ) {
            remove( reflection_filename );
        }
    }
...
void output_shader_stage( CodeGenerator* code_generator, const char* path, const Pass::ShaderStage& stage ) {
    ...
    append_finalized_shader_code( path, code_generator, stage, filename_buffer, code_buffer, constants_buffer, nullptr );
...
static void write_automatic_resources_layout( const hfx::Pass& pass, StringBuffer& pass_buffer, uint32_t& pass_offset ) {
    ...
    hydra::graphics::ResourceListLayoutCreation::Binding binding = { hydra::graphics::ResourceType::Constants, 0, 1, "MaterialConstants" };
    char* num_resources_data = pass_buffer.reserve( sizeof( uint8_t ) );

    uint8_t num_resources = 1;  // Local constants added
    pass_buffer.append_m( (void*)&binding, sizeof( hydra::graphics::ResourceListLayoutCreation::Binding ) );
...
static void write_vertex_input( const hfx::Pass& pass, StringBuffer& pass_buffer ) {
    ...
    pass_buffer.append_m( (void*)&pass.vertex_layout->attributes[0], sizeof( hydra::graphics::VertexAttribute ) * pass.vertex_layout->attributes.size() );
    pass_buffer.append_m( (void*)&pass.vertex_layout->streams[0], sizeof( hydra::graphics::VertexStream ) * pass.vertex_layout->streams.size() );
}
...
void code_generator_generate_embedded_file( CodeGenerator* code_generator, const char* output_filename ) {
    ...
    for ( uint32_t i = 0; i < pass_count; i++ ) {
        ...
        for ( size_t s = 0; s < pass_shader_stages; ++s ) {
            ...
            compilation_succeeded = compilation_succeeded && append_finalized_shader_code( input_path, code_generator, shader_stage, filename_buffer, shader_code_buffer, constants_buffer, reflection_file );
            ...
            if ( automatic_layout ) {
                for ( size_t p = 0; p < shader_stage.code->resources.size(); p++ ) {
                    ...
                    switch ( resource.type ) {
                        case hydra::graphics::ResourceType::ImageRW:
    ...
    if ( compilation_succeeded ) {
        ...
        if ( generate_reflection_data && reflection_file ) {
        ...
        hydra::print_format( "Error compiling shader %s. No binary generated.\n", input_path );
    }
}
...
//
//
bool hfx_compile( const char* input_filename, const char* output_filename, uint32_t options, ShaderEffectFile* out_shader_effect_file ) {
    char* text = hydra::file_read_into_memory( input_filename, nullptr, false, *hydra::memory_get_system_allocator() );
    if ( !text ) {
    ...
    hydra::file_remove_filename( input_path );
    ...
    code_generator_init( &code_generator, &parser, 256 * 1024, 8 );
    ...
    if ( out_shader_effect_file ) {
        shader_effect_init( *out_shader_effect_file, output_filename );
    }
...
void hfx_inspect( const char* binary_filename ) {
    char* text = hydra::file_read_into_memory( binary_filename, nullptr, false, *hydra::memory_get_system_allocator() );
    if ( !text ) {
        HYDRA_LOG( "Error compiling file %s: file not found.\n", binary_filename );
        return;
    }
...
void shader_effect_init( ShaderEffectFile& file, const char* full_filename ) {
    ...
    char* memory = hydra::file_read_into_memory( full_filename, nullptr, false, *hydra::memory_get_system_allocator() );
...
void shader_effect_shutdown( ShaderEffectFile& file ) {
    hydra::memory_get_system_allocator()->free_( file.memory );
}
...
static void shader_effect_pass_get_shader_creation( ShaderEffectFile::PassHeader* pass_header, uint32_t index, hydra::graphics::ShaderStateCreation::Stage* shader_creation ) {
...
//
void shader_effect_get_pipeline( ShaderEffectFile& hfx, uint32_t pass_index, hydra::graphics::PipelineCreation& out_pipeline ) {
...
//
void shader_effect_get_resource_list_layout( ShaderEffectFile& hfx, uint32_t pass_index, uint32_t layout_index, hydra::graphics::ResourceListLayoutCreation& out_list ) {
    ShaderEffectFile::PassHeader* pass_header = shader_effect_get_pass( hfx.memory, pass_index );

    uint8_t num_bindings;
    const hydra::graphics::ResourceListLayoutCreation::Binding* bindings = hfx::shader_effect_pass_get_layout_bindings( pass_header, layout_index, num_bindings );
...
//
void shader_effect_pass_get_pipeline( ShaderEffectFile::PassHeader* pass_header, hydra::graphics::PipelineCreation& out_pipeline ) {
    // get_shader_creation
    // get_vertex_input
    uint32_t shader_count = pass_header->num_shader_chunks;
    hydra::graphics::ShaderStateCreation& creation = out_pipeline.shaders;
    ...
    if ( pass_header->has_resource_state ) {
        ...
        memcpy( &out_pipeline.rasterization, render_state_memory, sizeof( hydra::graphics::RasterizationCreation ) + sizeof( hydra::graphics::DepthStencilCreation ) + sizeof( hydra::graphics::BlendStateCreation ) );
    }
