...
// 
void pipeline_create( Device& gpu, hfx::ShaderEffectFile& hfx, uint32_t pass_index, const RenderPassHandle& pass_handle, PipelineHandle& out_pipeline, ResourceListLayoutHandle* out_layouts, uint32_t num_layouts ) {
    ...
    for ( size_t i = 0; i < num_layouts; i++ ) {
        ...
        out_layouts[i] = gpu.create_resource_list_layout( rll_creation );

        // TODO: num active layout is already set to the max, so using add_rll breaks.
        render_pipeline.resource_list_layout[ i ] = out_layouts[ i ];
    }
...
void Material::load_resources( ShaderResourcesDatabase& db, Device& device ) {
    ...
    for ( size_t i = 0; i < array_size(passes); ++i ) {
        ...
        for ( uint32_t l = 0; l < pipeline_creation.num_active_layouts; ++l ) {
                ...
                switch ( binding.type ) {
                    ...
                    case hydra::graphics::ResourceType::Buffer:
                    {
                        ...
                        if ( !resource_name ) {
                            ...
                            resources_handles[r] = device.get_dummy_constant_buffer().handle;
                        } else {
                            ...
                            resources_handles[r] = handle.handle;
                        }
#else
                    ...
                    case hydra::graphics::ResourceType::Image:
                    {
                        ...
                        if ( !resource_name ) {
                            hydra::print_format( "Missing resource lookup for binding %s. Using dummy resource.\n", binding.name );
                            resources_handles[r] = device.get_dummy_texture().handle;
                        } else {
            ...
            creation.set_layout( pipeline_creation.resource_list_layout[l] ).set_resources( resources_handles, layout.num_active_bindings ).set_name(this->name);
...
void ShaderEffect2::shutdown( Device& gpu ) {
    ...
    for ( uint32_t i = 0; i < passes; ++i ) {
        ...
        gpu.destroy_resource_list_layout( resource_layouts[ i ] );
    }
}
...
void Material2::init( Device& gpu, ShaderEffect2* shader_, ResourceListCreation* resource_lists_ ) {
    ...
    shader = shader_;
    const u32 passes = shader->hfx_binary->header->num_passes;
    // First create arrays
    array_set_length( pipelines, passes );
    array_set_length( resource_lists, passes );
    array_set_length( compute_dispatches, passes );
...
void RenderStage2::render( Device& gpu, u64& sort_key, CommandBuffer* gpu_commands ) {
    ...
    switch ( type ) {
        ...
        case RenderPassType::Compute:
        {
            ...
            gpu_commands->dispatch( sort_key++, ceil( output_width * 1.f / dispatch.x ), ceil( output_height * 1.f / dispatch.y ), ceil( output_depth * 1.f / dispatch.z ) );

            barrier.set( PipelineStage::ComputeShader, PipelineStage::RenderTarget );
            gpu_commands->barrier( barrier );
            break;
        }
    ...
    */
    ...
    if ( update_projection ) {
        ...
        if ( perspective ) {
            ...
            projection = glms_ortho( -viewport_width / 2.f, viewport_width / 2.f, -viewport_height / 2.f, viewport_height / 2.f, near_plane, far_plane );
        }
    }
...
void Camera::yaw_pitch_from_direction( const vec3s& direction, f32& yaw, f32& pitch ) {
    ...
    yaw = glm_deg( atan2( direction.z, direction.x ) );
...
void GPUProfiler::update( Device& gpu ) {
    ...
    per_frame_active[ current_frame ] = active_timestamps;
    ...
    for ( uint32_t i = 0; i < active_timestamps; ++i ) {
        ...
        if ( color_index == 0xffff ) {
            ...
            color_index = string_hash_size( name_to_color );
...
void GPUProfiler::draw_ui() {

    {
        ...
        u32 rect_width = graph_width / max_frames;
        i32 rect_x = graph_width - rect_width;
        ...
        for ( u32 i = 0; i < max_frames; ++i ) {
            ...
            f32 frame_time = frame_timestamps[ 0 ].elapsed_ms;
            ...
            for ( u32 j = 0; j < per_frame_active[ frame_index ]; ++j ) {
                ...
                rect_height = timestamp.elapsed_ms / max_duration * widget_height;
        ...
        average_time = new_average / max_frames;
