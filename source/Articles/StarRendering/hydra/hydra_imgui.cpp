...
bool imgui_init( hydra::graphics::Device& graphics_device ) {
    ...
    ResourceListLayoutCreation resource_layout_creation {};
    ...
    pipeline_creation.add_resource_list_layout( g_resource_layout );
    ...
    rl_creation.set_layout( pipeline_creation.resource_list_layout[ 0 ] ).add_resource( g_ui_cb.handle ).add_resource( g_font_texture.handle ).set_name( "RL_ImGui" );
                ...
                if ( clip_rect.x < fb_width && clip_rect.y < fb_height && clip_rect.z >= 0.0f && clip_rect.w >= 0.0f )
                {
                    ...
                    if ( new_texture.handle != last_texture.handle ) {
                        ...
                        if ( last_resource_list.handle == k_invalid_handle ) {
                            ...
                            rl_creation.set_layout( g_resource_layout ).add_resource( g_ui_cb.handle ).add_resource( last_texture.handle ).set_name("RL_Dynamic_ImGUI");
