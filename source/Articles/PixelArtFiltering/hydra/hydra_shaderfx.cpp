            ...
            case 'p':
            {
                if ( lexer_expect_keyword( token.text, 6, "pragma" ) ) {
                    ...
                    if ( lexer_expect_keyword( new_token.text, 7, "include" ) ) {
                        ...
                        include.stage_mask = code_fragment.current_stage;
    ...
    while ( !lexer_equals_token( parser->lexer, token, Token::Token_CloseBrace ) ) {
        ...
        if ( token.type == Token::Token_Identifier ) {
            ...
            if ( lexer_expect_keyword( token.text, 4, "Cull" ) ) {
                ...
                hydra::graphics::BlendState& bs = render_state.blend_state.blend_states[ render_state.blend_state.active_states ];

                if ( lexer_expect_keyword( token.text, 5, "Alpha" ) ) {    
                    bs.blend_enabled = 1;
                    bs.color_operation = hydra::graphics::BlendOperation::Add;
                    bs.source_color = hydra::graphics::Blend::SrcAlpha;
                    bs.destination_color = hydra::graphics::Blend::InvSrcAlpha;
                }
                else if ( lexer_expect_keyword( token.text, 13, "Premultiplied" ) ) {
                    bs.blend_enabled = 1;
                    bs.color_operation = hydra::graphics::BlendOperation::Add;
                    bs.source_color = hydra::graphics::Blend::One;
                    bs.destination_color = hydra::graphics::Blend::InvSrcAlpha;
                }
...
void declaration_includes( Parser* parser ) {
    ...
    while ( !lexer_equals_token( parser->lexer, token, Token::Token_CloseBrace ) ) {
        ...
        if ( token.type == Token::Token_String ) {
            ...
            char* text = hydra::file_read_binary( path_buffer.data, *hydra::memory_get_system_allocator(), nullptr );
            if ( text ) {
                Lexer lexer;
...
//
//
static void append_reflection_data( const char* reflection_filename, const char* namespace_name, StringBuffer* reflection_buffer, TypeDefinitionAlias* name_to_type, StringBuffer& buffer ) {

    if ( reflection_buffer ) {

        // Parse json
        using json = nlohmann::json;
        char* reflection_json = hydra::file_read_text( reflection_filename, *hydra::memory_get_system_allocator(), nullptr );
                ...
                if ( offset.is_number() ) {
                    ...
                    if ( member_memory_offset < current_member_offset ) {
                        while ( member_memory_offset < current_member_offset ) {
                            ...
                            reflection_buffer->append_f( "\t\t\t pad%3d;\n", padding_added++ );
                        }
                    }
...
static void append_include_code( const CodeFragment::Include& include, cstring path, const Parser* parser, Stage& stage, StringBuffer& filename_buffer, StringBuffer& code_buffer ) {
    ...
    if ( include.file_or_local == 1 ) {
        ...
        char* include_code = hydra::file_read_text( filename_buffer.data, *hydra::memory_get_system_allocator(), nullptr );
        if ( include_code ) {
...
static bool append_finalized_shader_code( const char* path, const CodeGenerator* code_generator, const Pass::ShaderStage& shader_stage,
    ...
    while ( lexer.position - code_fragment->code.text < code_fragment->code.length ) {
    ...
    if ( (compile_options & CompileOptions_SpirV) == CompileOptions_SpirV ) {
        ...
        final_shader = hydra::file_read_binary( final_shader_filename, *hydra::memory_get_system_allocator(), &final_shader_size );
    }
    else {
        // Convert to SpirV binary blob.
        char* glsl_compiler_path = filename_buffer.append_use_f( "%sglslangValidator.exe", code_generator->shader_binaries_path );
    ...
    if ( !compilation_succeeded ) {
        // Print the shader
        char* shader_code = hydra::file_read_text( temp_filename, *hydra::memory_get_system_allocator(), nullptr );
        ...
        i32 min_line = max( 0, error_line - k_output_error_lines );
        // Search line
        ...
        for ( size_t i = 0; i < k_output_error_lines; i++ ) {
            ...
            hydra::print_format( "%s%s", ( i == k_output_error_lines - 1 ) ? "\nERROR LINE:\n" : "", shader_line_text );
    ...
    if ( generate_reflection_data ) {
        ...
        append_reflection_data( reflection_filename, namespace_name, reflection_buffer, code_generator->name_to_type, filename_buffer );
    }
...
static void write_automatic_resources_layout( const hfx::Pass& pass, StringBuffer& pass_buffer, uint32_t& pass_offset ) {
    ...
    hydra::graphics::ResourceLayoutCreation::Binding binding = { hydra::graphics::ResourceType::Constants, 0, 1, "MaterialConstants" };
    char* num_resources_data = pass_buffer.reserve( sizeof( uint8_t ) );

    uint8_t num_resources = 1;  // Local constants added
    pass_buffer.append_m( (void*)&binding, sizeof( hydra::graphics::ResourceLayoutCreation::Binding ) );
    pass_offset += sizeof( hydra::graphics::ResourceLayoutCreation::Binding ) + sizeof( uint8_t );
        ...
        for ( size_t p = 0; p < shader_stage.code->resources.size(); p++ ) {
            ...
            switch ( resource.type ) {
                case ResourceType::Texture:
                {
                    ...
                    pass_buffer.append_m( (void*)&binding, sizeof( hydra::graphics::ResourceLayoutCreation::Binding ) );
                    pass_offset += sizeof( hydra::graphics::ResourceLayoutCreation::Binding );
                    ++num_resources;
...
bool hfx_compile( const char* input_filename, const char* output_filename, uint32_t options, ShaderEffectFile* out_shader_effect_file ) {
    char* text = hydra::file_read_text( input_filename, *hydra::memory_get_system_allocator(), nullptr );
    if ( !text ) {
    ...
    // If so don't compile.
    if ( hydra::file_exists( output_filename ) ) {
        ShaderEffectFile::Header file_header;
...
void hfx_inspect( const char* binary_filename ) {
    char* text = hydra::file_read_binary( binary_filename, *hydra::memory_get_system_allocator(), nullptr );
    if ( !text ) {
        HYDRA_LOG( "Error compiling file %s: file not found.\n", binary_filename );
        return;
    }
...
void shader_effect_init( ShaderEffectFile& file, const char* full_filename ) {
    ...
    char* memory = hydra::file_read_binary( full_filename, *hydra::memory_get_system_allocator(), nullptr );
...
//
void shader_effect_get_resource_list_layout( ShaderEffectFile& hfx, uint32_t pass_index, uint32_t layout_index, hydra::graphics::ResourceLayoutCreation& out_list ) {
    ShaderEffectFile::PassHeader* pass_header = shader_effect_get_pass( hfx.memory, pass_index );

    uint8_t num_bindings;
    const hydra::graphics::ResourceLayoutCreation::Binding* bindings = hfx::shader_effect_pass_get_layout_bindings( pass_header, layout_index, num_bindings );
...
void shader_effect_get_resource_layout( ShaderEffectFile& hfx, u32 pass_index, u32 layout_index, hydra::graphics::ResourceLayoutCreation& out_list, NameToIndex* out_map ) {
    ShaderEffectFile::PassHeader* pass_header = shader_effect_get_pass( hfx.memory, pass_index );
    ...
    for ( u16 i = 0; i < num_bindings; i++ ) {
        ...
        const hydra::graphics::ResourceLayoutCreation::Binding& binding = bindings[ i ];
...
//
//
const hydra::graphics::ResourceLayoutCreation::Binding* shader_effect_pass_get_layout_bindings( ShaderEffectFile::PassHeader* pass_header, uint32_t layout_index, uint8_t& out_num_bindings ) {
    ...
    while ( layout_index-- ) {
        ...
        pass_memory += (sizeof( uint8_t ) + num_bindings * sizeof( hydra::graphics::ResourceLayoutCreation::Binding ) );
    }

    // Retrieve bindings count
    out_num_bindings = (uint8_t)*pass_memory;
    // Returns the bindings.
    return (const hydra::graphics::ResourceLayoutCreation::Binding*)(pass_memory + sizeof( uint8_t ));
}
