...
void ResourceManager::terminate( hydra::graphics::Device& gfx_device ) {
    ...
    for ( size_t i = 0; i < string_hash_size( name_to_resources ); ++i ) {
...
void ResourceManager::init_resource( Resource** resource, char* memory, hydra::graphics::Device& gfx_device, hydra::graphics::RenderFrame* render_pipeline ) {
...
Resource* ResourceManager::compile_resource( ResourceType::Enum type, const char* filename ) {
    ...
    char* source_file_memory = hydra::file_read_into_memory( source_full_filename, &file_size );
...
Resource* ResourceManager::load_resource( ResourceType::Enum type, const char* filename, hydra::graphics::Device& gfx_device, hydra::graphics::RenderPipeline* render_pipeline ) {
    ...
    char* file_memory = hydra::file_read_into_memory( resource_full_filename, nullptr );
    if ( !file_memory ) {
...
void ShaderFactory::compile_resource( CompileContext& context ) {
    ...
    hfx::hfx_compile( hfx_full_filename, context.resource_manager->get_resource_binary_folder(), bhfx_filename );

#elif defined(HYDRA_VULKAN)

    char* arguments = context.temp_string_buffer.append_use( "HydraShaderFX_Debug.exe %s -V -o %s -b", hfx_full_filename, context.compiled_filename );
    hydra::execute_process( "..\\bin", "C:\\Coding\\github\\HydraShaderFX\\Bin\\HydraShaderFX_Debug.exe", arguments );

#endif // HYDRA_VULKAN

    // Read the newly generated bhfx file
    char* bhfx_memory = hydra::file_read_into_memory( context.compiled_filename, &context.out_header->data_size, false );
...
void* ShaderFactory::load( LoadContext& context ) {
    ...
    hfx::shader_effect_init( shader_effect_file, context.resource->data );
    ...
    for ( uint16_t p = 0; p < effect->num_passes; p++ ) {
        hfx::ShaderEffectFile::PassHeader* pass_header = hfx::shader_effect_get_pass( shader_effect_file.memory, p );
        ...
        for ( uint16_t l = 0; l < pass_header->num_resource_layouts; l++ ) {
            ...
            const ResourceListLayoutCreation::Binding* bindings = hfx::shader_effect_pass_get_layout_bindings( pass_header, l, num_bindings );
    ...
    if ( !invalid_effect ) {
        ...
        for ( uint32_t p = 0; p < effect->num_properties; ++p ) {
            hfx::ShaderEffectFile::MaterialProperty* property = hfx::shader_effect_get_property( effect->properties_data, p );
...
void ShaderFactory::reload( Resource* old_resource, Resource* new_resource, StringBuffer& temp_string_buffer, hydra::graphics::Device& gfx_device, hydra::graphics::RenderPipeline* render_pipeline ) {
    ...
    hfx::ShaderEffectFile shader_effect_file;
    hfx::shader_effect_init( shader_effect_file, new_resource->data );

    effect->init( shader_effect_file );
    
    for ( uint16_t p = 0; p < effect->num_passes; p++ ) {
        hfx::ShaderEffectFile::PassHeader* pass_header = hfx::shader_effect_get_pass( new_resource->data, p );
...
void* MaterialFactory::load( LoadContext& context ) {
    ...
    material->shader_instances = new MaterialPass[shader_effect->num_passes];
    ...
    for ( size_t p = 0; p < material_file.header->num_properties; ++p ) {
        ...
        switch ( material_property->type ) {
            case hfx::PropertyType::Texture2D:
            {
...
//#define MATERIAL_DEBUG_BINDINGS
    ...
    uint32_t num_map_entries = string_hash_size_u( resource_lookup.binding_to_resource );
    ...
    for ( uint8_t p = 0; p < shader_effect->num_passes; ++p ) {
        ...
        if ( stage ) {
            ...
            stage->material_pass_index = (uint8_t)p;
        }
    }
