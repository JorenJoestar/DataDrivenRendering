...
// Revision history //////////////////////
//
//      0.37  (2020/01/28): + Fixed unordered binding of resource list based on index.
//      0.36  (2020/01/26): + Added RenderPassOutput class to separate between Pipeline texture format needs and actual binding.
//      0.35  (2020/01/23): + Fixed recreation of render passes by deferring resource deletion with temporary textures/render passes to be deleted.
//      0.34  (2020/01/21): + BREAKING: renamed add_buffer and add_texture methods to simply 'buffer' and 'texture'. + Added binding to methods.
//      0.33  (2020/01/20): + Fixed GPU queries result wrong on non 0 frames.
//      0.32  (2020/01/14): + BREAKING: renamed resource_list_layout to resource_layout.
//      0.31  (2020/01/07): + Added internal recreation of swapchain during resize to maintain same resource handle.
//      0.30  (2020/01/02): + BREAKING: renamed handle member in handle to index. + Added invalid handles for resources.
//      0.29  (2020/01/02): + BREAKING: added typed handles to resource lists. + BIG: added DEFERRED update resource list method. + Fixed iteration for resource deletion.
...
namespace ResourceDeletionType {
    ...
    enum Enum {
        Buffer, Texture, Pipeline, Sampler, ResourceLayout, ResourceList, RenderPass, ShaderState, Count
    };
} // namespace ResourceDeletionType
...
struct DeviceRenderFrame {
    ...
    CommandBuffer*                  get_instant_command_buffer( u32 thread_index );
    ...
    u32                             command_buffers_per_thread = 2;
    ...
    u8*                             command_buffer_per_thread_index = nullptr;  // Cache buffers to get next command buffer.
