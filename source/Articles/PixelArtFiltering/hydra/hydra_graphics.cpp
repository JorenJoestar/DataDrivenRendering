...
//#define HYDRA_MULTITHREADING
    ...
    #define HYDRA_MALLOC( size )                                halloc(size)
    #define HYDRA_FREE( pointer )                               hfree(pointer)
...
void ResourcePool::init( u32 pool_size, u32 resource_size_ ) {

    this->size = pool_size;
    this->resource_size = resource_size_;

    memory = (u8*)HYDRA_MALLOC( pool_size * resource_size );

    // Allocate and add free indices
    free_indices = (u32*)HYDRA_MALLOC( pool_size * sizeof( u32 ) );
    free_indices_head = 0;
...
ShaderStateHandle Device::create_shader_state( const ShaderStateCreation& creation ) {
    ...
    if ( handle.handle == k_invalid_index ) {
        return handle;
    }
...
PipelineHandle Device::create_pipeline( const PipelineCreation& creation ) {
    ...
    if ( shader_state.handle == k_invalid_index ) {
...
void Device::destroy_buffer( BufferHandle buffer ) {
    if ( buffer.handle != k_invalid_index ) {
...
void Device::destroy_texture( TextureHandle texture ) {
    ...
    if ( texture.handle != k_invalid_index ) {
...
void Device::destroy_shader_state( ShaderStateHandle shader ) {
    ...
    if ( shader.handle != k_invalid_index ) {
...
void Device::destroy_pipeline( PipelineHandle pipeline ) {
    if ( pipeline.handle != k_invalid_index ) {
...
void Device::destroy_sampler( SamplerHandle sampler ) {
    if ( sampler.handle != k_invalid_index ) {
...
void Device::destroy_resource_list_layout( ResourceListLayoutHandle resource_layout ) {
    if ( resource_layout.handle != k_invalid_index ) {
...
void Device::destroy_resource_list( ResourceListHandle resource_list ) {
    if ( resource_list.handle != k_invalid_index ) {
...
void Device::destroy_render_pass( RenderPassHandle render_pass ) {
    if ( render_pass.handle != k_invalid_index ) {
...
void Device::query_resource_list_layout( ResourceListLayoutHandle resource_list_layout, ResourceListLayoutDescription& out_description ) {
    if ( resource_list_layout.handle != k_invalid_index ) {
...
void* Device::map_buffer( const MapBufferParameters& parameters ) {
    if ( parameters.buffer.handle == k_invalid_index )
        return nullptr;
...
void create_fbo( const RenderPassCreation& creation, RenderPassGL& render_pass, Device& device ) {
    ...
    if ( creation.depth_stencil_texture.handle != k_invalid_index ) {
...
void Device::backend_init( const DeviceCreation& creation ) {
...
#ifdef VULKAN_DEBUG_REPORT
    {
        ...
        VkExtensionProperties* extensions = ( VkExtensionProperties* )HYDRA_MALLOC( sizeof( VkExtensionProperties ) * num_instance_extensions );
    ...
    VkPhysicalDevice* gpus = (VkPhysicalDevice*)HYDRA_MALLOC( sizeof( VkPhysicalDevice ) * num_physical_device );
    ...
    VkQueueFamilyProperties* queue_families = (VkQueueFamilyProperties*)HYDRA_MALLOC( sizeof( VkQueueFamilyProperties ) * queue_family_count );
    ...
    VkSurfaceFormatKHR* supported_formats = (VkSurfaceFormatKHR*)HYDRA_MALLOC( sizeof( VkSurfaceFormatKHR ) * supported_count );
    ...
    VkPresentModeKHR* supported_modes = (VkPresentModeKHR*)HYDRA_MALLOC( sizeof( VkPresentModeKHR ) * supported_count );
    ...
    for ( ; mode < modes_count; mode++ ) {
        for ( uint32_t j = 0; j < supported_count; j++ ) {
            if ( present_modes[mode] == supported_modes[j] ) {
                mode_found = true;
                break;
            }
        }
    ...
    HYDRA_FREE( supported_modes );

    vulkan_present_mode = mode >= modes_count ? VK_PRESENT_MODE_FIFO_KHR : present_modes[mode];
    ...
    resource_layouts.init( 128, sizeof( ResourceLayoutVulkan ) );
    ...
    render_frames = (DeviceRenderFrame*)HYDRA_MALLOC( sizeof( DeviceRenderFrame ) * k_max_frames );
    ...
    swapchain_output.depth( TextureFormat::D24_UNORM_X8_UINT );
    ...
    queued_command_buffers = (CommandBuffer**)HYDRA_MALLOC( sizeof( CommandBuffer* ) * 128 );
    ...
    gpu_timestamp_manager = ( GPUTimestampManager* )HYDRA_MALLOC( sizeof( GPUTimestampManager ) );
...
void Device::backend_terminate() {
    ...
    HYDRA_FREE( gpu_timestamp_manager );

    HYDRA_FREE( queued_command_buffers );
    HYDRA_FREE( render_frames );
...
ShaderStateHandle Device::create_shader_state( const ShaderStateCreation& creation ) {
    ...
    for ( compiled_shaders = 0; compiled_shaders < creation.stages_count; ++compiled_shaders ) {
        ...
        if ( creation.spv_input ) {
            ...
            // Read back SPV file.
            createInfo.pCode = reinterpret_cast<const uint32_t*>(hydra::file_read_binary( final_shader_filename, *memory_get_system_allocator(), &createInfo.codeSize ) );
...
//
//
static void vulkan_fill_write_descriptor_sets( Device& gpu, const ResourceLayoutVulkan* resource_layout, VkDescriptorSet vk_descriptor_set,
    ...
    for ( u32 r = 0; r < num_resources; r++ ) {
        ...
        u32 i = bindings[ r ];
        const auto& binding = resource_layout->bindings[ i ];
...
ResourceListHandle Device::create_resource_list( const ResourceListCreation& creation ) {
    ...
    // Cache data
    resource_list->resources = ( ResourceHandle* )HYDRA_MALLOC( sizeof( ResourceHandle ) * creation.num_resources );
    resource_list->samplers = ( SamplerHandle* )HYDRA_MALLOC( sizeof( SamplerHandle ) * creation.num_resources );
    resource_list->bindings = ( u16* )HYDRA_MALLOC( sizeof( u16 ) * creation.num_resources );
...
void Device::update_resource_list( const ResourceListUpdate& update ) {
    ...
    if ( update.resource_list.index < resource_lists.size ) {

        resource_list_update_queue[ num_update_queue ] = update;
        resource_list_update_queue[ num_update_queue ].frame_issued = current_frame;
        ++num_update_queue;
    } else {
        print_format( "Graphics error: trying to update invalid ResourceList %u\n", update.resource_list.index );
    }
}

void Device::update_resource_list_instant( const ResourceListUpdate& update ) {
...
static void vulkan_create_swapchain_pass( Device& gpu, const RenderPassCreation& creation, RenderPassVulkan* render_pass ) {
    // Color attachment
...
//
//
static VkRenderPass vulkan_create_render_pass( Device& gpu, const RenderPassOutput& output, cstring name ) {
...
//
//
static RenderPassOutput find_render_pass_output( Device& gpu, const RenderPassCreation& creation ) {
    RenderPassOutput output;
    output.reset();
...
RenderPassHandle Device::create_render_pass( const RenderPassCreation& creation ) {
    ...
    switch ( creation.type ) {
        ...
        {
            render_pass->output = find_render_pass_output( *this, creation );
...
void Device::destroy_texture( TextureHandle texture ) {
    if ( texture.index < textures.size ) {
...
void Device::destroy_pipeline( PipelineHandle pipeline ) {
    if ( pipeline.index < pipelines.size ) {
...
void Device::destroy_sampler( SamplerHandle sampler ) {
    if ( sampler.index < samplers.size ) {
...
void Device::destroy_resource_layout( ResourceLayoutHandle resource_layout ) {
    if ( resource_layout.index < resource_layouts.size ) {
        resource_deletion_queue[ num_deletion_queue++ ] = { ResourceDeletionType::ResourceLayout, resource_layout.index, current_frame };
    }
    else {
        print_format( "Graphics error: trying to free invalid ResourceLayout %u\n", resource_layout.index );
    }
}

void Device::destroy_resource_list( ResourceListHandle resource_list ) {
    if ( resource_list.index < resource_lists.size ) {
...
void Device::destroy_render_pass( RenderPassHandle render_pass ) {
    if ( render_pass.index < render_passes.size ) {
...
void Device::destroy_resource_layout_instant( ResourceHandle resource_layout ) {
...
void Device::destroy_resource_list_instant( ResourceHandle resource_list ) {
    ...
    if ( v_resource_list ) {
        HYDRA_FREE( v_resource_list->resources );
        HYDRA_FREE( v_resource_list->samplers );
        HYDRA_FREE( v_resource_list->bindings );
...
VkRenderPass Device::get_vulkan_render_pass( const RenderPassOutput& output, cstring name ) {

    return vulkan_create_render_pass( *this, output, name );
}
...
CommandBuffer* Device::get_instant_command_buffer() {
    CommandBuffer* cb = render_frames[ current_frame ].get_instant_command_buffer( 0 );
    return cb;
}
...
//
//
static void vulkan_resize_texture( Device& gpu, TextureVulkan* v_texture, TextureVulkan* v_texture_to_delete, u16 width, u16 height, u16 depth ) {
...
void Device::present() {
    ...
    for ( u32 i = 0; i < 3; i++ ) {
        if ( vkGetFenceStatus( vulkan_device, render_frames[i].vulkan_in_flight_fence ) == VK_SUCCESS && (i != current_frame) ) {
            free_fence_index = i;
            break;
        }
    }
    ...
    if ( num_deletion_queue > 0 ) {
        for ( i32 i = num_deletion_queue - 1; i >= 0; i-- ) {
            ...
            if ( resource_deletion.current_frame == free_fence_index ) {
    ...
    if ( num_update_queue ) {
        for ( i32 i = num_update_queue - 1; i >= 0; i-- ) {
            ...
            if ( update.frame_issued == free_fence_index ) {
    ...
    for ( uint32_t l = 0; l < num_lists; ++l ) {
        ...
        for ( u32 i = 0; i < resource_layout->num_bindings; ++i ) {
            ...
            if ( rb.type == ResourceType::Constants ) {
                ...
                ResourceHandle handle = resource_list->resources[ resource_index ];
...
void DeviceRenderFrame::init( Device* gpu_device_, u32 thread_count_ ) {
    thread_count = thread_count_;
    gpu_device = gpu_device_;
    command_buffers_per_thread = 2;
    ...
    const u32 max_command_buffers = thread_count_ * command_buffers_per_thread;

    // Allocate the arrays
    vulkan_command_pools = (VkCommandPool*)HYDRA_MALLOC( sizeof( VkCommandPool ) * thread_count_ );
    vulkan_command_buffers = (VkCommandBuffer*)HYDRA_MALLOC( sizeof( VkCommandBuffer ) * max_command_buffers );
    command_buffers = (CommandBuffer**)HYDRA_MALLOC( sizeof( CommandBuffer* ) * max_command_buffers );
    command_buffer_per_thread_index = ( u8* )HYDRA_MALLOC(sizeof(u8) * thread_count_);
    ...
    for ( size_t i = 0; i < thread_count; i++ ) {
        ...
        cmd.commandBufferCount = command_buffers_per_thread;
        check( vkAllocateCommandBuffers( gpu_device->vulkan_device, &cmd, vulkan_command_buffers ) );

        for ( size_t c = 0; c < command_buffers_per_thread; c++ ) {
        ...
        // Reset per thread indices
        command_buffer_per_thread_index[ i ] = 0;
    }
...
void DeviceRenderFrame::terminate() {
    ...
    HYDRA_FREE(vulkan_command_pools);
    HYDRA_FREE(vulkan_command_buffers);
    HYDRA_FREE(command_buffers);
    HYDRA_FREE(command_buffer_per_thread_index);
}

CommandBuffer* DeviceRenderFrame::get_command_buffer( u32 thread_index, bool begin ) {

    CommandBuffer* command_buffer = command_buffers[thread_index * command_buffers_per_thread];
...
CommandBuffer* DeviceRenderFrame::get_instant_command_buffer( u32 thread_index ) {
    // Get the instant command buffer allocated always after the cb per thread.
    CommandBuffer* command_buffer = command_buffers[ thread_index * command_buffers_per_thread + 1 ];
    command_buffer->reset();
    return command_buffer;
}
...
void DeviceRenderFrame::on_resize() {
    ...
    /*const u32 max_command_buffers = thread_count * command_buffers_per_thread;

    for ( size_t i = 0; i < thread_count; i++ ) {
        vkFreeCommandBuffers( gpu_device->vulkan_device, vulkan_command_pools[i], command_buffers_per_thread, vulkan_command_buffers );
