...
namespace hydra {
    ...
    // Retrieve path without the filename
    ...
    #define halloc(size)            (hydra::memory_get_system_allocator()->allocate( size, 1, __FILE__, __LINE__ ))
    #define halloca(size, allocator) ((allocator).allocate( size, 1, __FILE__, __LINE__ ))

    #define hfree(pointer)          hydra::memory_get_system_allocator()->free_(pointer)
    #define hfreea(pointer, allocator) (allocator).free_(pointer)
